...
function startStructuredEntry(type){
  structuredEntry=(type==='integral'||type==='summation'||type==='product')?{type,lower:'',upper:'',formula:expr||'',field:'formula'}:{type,formula:expr||'',point:'',field:'formula'};
  expr='';resultShown=false;updateDisplay()
}
function structuredFields(){return ['integral','summation','product'].includes(structuredEntry.type)?['lower','upper','formula']:['formula','point']}
