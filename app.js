...
function livePreview(){if(!expr||/[XYi]/.test(expr))return '';try{const source=closeParentheses(expr.replace(/(\d+(?:\.\d+)?)%(?![\d.(])/g,'($1/100)')),value=safeEval(source);return Number.isFinite(value)?String(+value.toPrecision(12)):''}catch{return ''}}
