...
function updateNumericFormatUI(){const label=$('#numberFormatLabel');if(label)label.textContent=numberFormat;const decimal=document.querySelector('[data-action="."]');if(decimal){const symbol=numberFormat==='BR'?',':'.';decimal.textContent=symbol;decimal.dataset.primaryLabel=symbol}}
function updateAngleMode(){const deg=angleMode==='DEG';$('#degBtn').classList.toggle('active',deg);$('#radBtn').classList.toggle('active',!deg);$('#modeLabel').textContent=angleMode;updateNumericFormatUI()}
$('#degBtn').onclick=()=>{angleMode='DEG';localStorage.setItem('wosvipAngleMode',angleMode);updateAngleMode()};$('#radBtn').onclick=()=>{angleMode='RAD';localStorage.setItem('wosvipAngleMode',angleMode);updateAngleMode()};
function showModeSettings(){document.querySelector('.mode-settings-overlay')?.remove();const overlay=document.createElement('div');overlay.className='converter-overlay mode-settings-overlay';overlay.innerHTML='<div class="converter-dialog mode-settings-dialog" role="dialog" aria-modal="true"><h3>Configuração da calculadora</h3><div class="mode-setting-group"><span>Ângulo</span><div><button type="button" data-angle="DEG">DEG</button><button type="button" data-angle="RAD">RAD</button></div></div><div class="mode-setting-group"><span>Formato numérico</span><div><button type="button" data-format="BR">BR&nbsp; 1.234,56</button><button type="button" data-format="US">US&nbsp; 1,234.56</button></div></div><button type="button" class="mode-settings-close">Concluir</button></div>';document.body.appendChild(overlay);const refresh=()=>{overlay.querySelectorAll('[data-angle]').forEach(button=>button.classList.toggle('selected',button.dataset.angle===angleMode));overlay.querySelectorAll('[data-format]').forEach(button=>button.classList.toggle('selected',button.dataset.format===numberFormat))};overlay.querySelectorAll('[data-angle]').forEach(button=>button.onclick=()=>{angleMode=button.dataset.angle;localStorage.setItem('wosvipAngleMode',angleMode);updateAngleMode();refresh()});overlay.querySelectorAll('[data-format]').forEach(button=>button.onclick=()=>{numberFormat=button.dataset.format;localStorage.setItem('wosvipNumberFormat',numberFormat);updateNumericFormatUI();updateDisplay();refresh()});const close=()=>overlay.remove();overlay.querySelector('.mode-settings-close').onclick=close;overlay.onclick=event=>{if(event.target===overlay)close()};refresh()}
updateAngleMode();
...
numberButtons.forEach((k,i)=>makeKey($('#numberKeys'),k,i===19?'enter-key':i===18||i%5<3?'number-key':'operator-key'));updateNumericFormatUI();
...
function structuredFields(){if(structuredEntry.type==='inverse')return ['formula'];if(structuredEntry.type==='yroot')return ['index','radicand'];return ['integral','summation','product'].includes(structuredEntry.type)?['lower','upper','formula']:['formula','point']}
...
function localizeNumericToken(token){const parts=token.split('.'),group=numberFormat==='BR'?'.':',',decimal=numberFormat==='BR'?',':'.',integer=parts[0].replace(/\B(?=(\d{3})+(?!\d))/g,group);return parts.length>1?integer+decimal+parts.slice(1).join(''):integer}
function localizeExpressionSource(value){return String(value??'').replace(/\d+(?:\.\d*)?/g,localizeNumericToken)}
function formatLocalizedMath(value){return formatMath(localizeExpressionSource(value))}
function updateDisplay(){cursorPosition=Math.max(0,Math.min(cursorPosition,expr.length));const display=$('#expression');if(structuredEntry){display.innerHTML=renderStructuredEntry();$('#screen').value='';updateStepsButton();const info=display.querySelector('.limit-info-button');if(info)info.onclick=event=>{event.stopPropagation();showLimitInfo()};return}if(resultShown)display.innerHTML=lastFormula?formatLocalizedMath(lastFormula):'Pronto';else if(expr)display.innerHTML=formatLocalizedMath(expr.slice(0,cursorPosition))+'<span class="edit-caret" aria-hidden="true"></span>'+formatLocalizedMath(expr.slice(cursorPosition));else display.innerHTML='<span class="edit-caret" aria-hidden="true"></span>';const preview=resultShown?expr:livePreview();$('#screen').value=preview?localizeExpressionSource(preview):'';updateStepsButton();requestAnimationFrame(()=>{const caret=display.querySelector('.edit-caret');if(caret)caret.scrollIntoView({block:'nearest',inline:'nearest'})})}updateDisplay();$('#resultStepsBtn').onclick=showCalculationSteps;
...
updateInstallButton();
function flashPhysicalKey(action){const button=document.querySelector('[data-action="'+CSS.escape(String(action))+'"]');if(!button)return;button.classList.remove('physical-key-active');void button.offsetWidth;button.classList.add('physical-key-active');setTimeout(()=>button.classList.remove('physical-key-active'),120)}
document.addEventListener('keydown',event=>{const target=event.target;if(target&&(/^(INPUT|TEXTAREA|SELECT)$/.test(target.tagName)||target.isContentEditable))return;if(document.querySelector('.converter-overlay'))return;let action=null;const key=event.key,code=event.code;if(/^\d$/.test(key))action=key;else if(key==='+'||key==='-'||key==='*'||key==='/'||key==='('||key===')'||key==='^')action=key;else if(key==='Enter'||key==='=')action='equal';else if(key==='Backspace')action='back';else if(key==='Delete'||key==='Escape')action='clear';else if(key==='x'||key==='X')action='variable';else if(code==='NumpadDecimal')action='.';else if(numberFormat==='BR'&&key===',')action='.';else if(numberFormat==='US'&&key==='.')action='.';else if((numberFormat==='BR'&&key==='.')||(numberFormat==='US'&&key===',')){event.preventDefault();return}if(action===null)return;event.preventDefault();flashPhysicalKey(action);press(action)});
