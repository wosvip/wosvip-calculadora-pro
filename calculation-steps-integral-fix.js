...
(function(){
  ...
  function monomialSteps(q,result){
    ...
    const ruleGeneral=`∫ aX^n dX = a × X^(n + 1)/(n + 1)`;
    const ruleApplied=`∫ ${coeffText}X^${powerText} dX = ${coeffText} × X^(${powerText} + 1)/(${powerText} + 1)`;
    const ruleSimplified=`= ${coeffText} × X^${nextText}/${nextText} = ${primitive}`;
    return [
      ...
      {title:"Aplique a regra da potência",html:stepMath(ruleGeneral)+stepMath(ruleApplied)+stepMath(ruleSimplified)},
